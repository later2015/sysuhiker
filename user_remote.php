...
class user_remote{
    ...
    //给认证队员发送活动消息
     public function sendWxNotice($content='test')
    {
            //$fakeId标识了你给谁发消息
    $fakeId = $this->getFakeId(100);//100是认证会员的组ID
    //$content = "这是我主动发的消息哦";
    foreach($fakeId as $obj){
         $res = $this->sendTextMsg($obj->fakeId,$content);
         //print_r($res);
    }
    //向未分组的会员发送消息
     $fakeId0 = $this->getFakeId(0);//0是未会员的组ID
    foreach($fakeId0 as $obj){
         $res = $this->sendTextMsg($obj->fakeId,$content);
    }
